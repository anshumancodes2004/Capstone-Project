...
# Campus IP ranges — .env mein CAMPUS_IP_RANGES="10.104.242,192.168.1." set karo
# Multiple ranges comma se separate karo
...
def get_first_name(full_name):
    """Full name se sirf pehla word — 'Anshuman Kumar Singh' → 'Anshuman'"""
    if not full_name:
        return "there"
...
def generate_result_pdf(student, exam, answers, total_score, percentage):
    ...
    elif pct >= 75: grade = 'A (Excellent)'
    elif pct >= 60: grade = 'B (Good)'
    elif pct >= 45: grade = 'C (Average)'
    elif pct >= 35: grade = 'D (Pass)'
    else:           grade = 'F (Fail)'
    summary_data = [['Metric','Value'],['Total Questions',str(total_q)],['Attempted',str(attempted)],['Not Attempted',str(not_attempted)],['Total Score',f'{total_score} / {max_marks}'],['Percentage',f'{pct}%'],['Grade',grade]]
    ...
    if pct >= 75:
        overall_fb = f"Excellent performance! The student demonstrated strong understanding of the subject matter, scoring {pct}%. Keep up the great work."
    elif pct >= 50:
        overall_fb = f"Good attempt. The student scored {pct}% showing adequate understanding. Review the questions where marks were deducted to improve further."
    elif pct >= 35:
        overall_fb = f"The student scored {pct}%. While the basic concepts are understood, more practice and deeper study of the subject is recommended."
    else:
        overall_fb = f"The student scored {pct}%. Significant improvement is needed. Please review the course material thoroughly and seek guidance."
    fb_table = Table([[Paragraph(overall_fb, normal_style)]], colWidths=[17*cm])
    ...
    story.append(Paragraph(f"Generated by OEMS — Online Examination Management System | {datetime.now().strftime('%d %b %Y, %I:%M %p')}", ParagraphStyle('Footer', parent=muted_style, alignment=TA_CENTER, fontSize=8)))
    doc.build(story)
    buffer.seek(0)
    return buffer.read()
...
def evaluate_answer(question, student_answer, max_marks, model_answer=None):
    ...
    stop_words = {'what','is','are','the','a','an','of','in','to','and','or','for','with','how','why','explain','describe','define','write','give','list','state'}
    ...
    if sbert_model is not None:
        try:
            ...
            sbert_score = float(cosine_similarity([embeddings[0]], [embeddings[1]])[0][0])
        ...
        sbert_score = (keyword_score + tfidf_score) / 2
    combined = (sbert_score * 0.60 + tfidf_score * 0.25 + keyword_score * 0.15)
    combined = max(0.0, min(1.0, combined))
    if word_count >= 80:   combined = min(1.0, combined + 0.05)
    elif word_count >= 40: combined = min(1.0, combined + 0.02)
    raw_score = combined * float(max_marks)
    score     = round(raw_score * 2) / 2
    if combined >= 0.75:   feedback = f"Excellent answer. Concepts clearly explained. Score: {score}/{max_marks}."
    elif combined >= 0.55: feedback = f"Good attempt. Key concepts covered. Score: {score}/{max_marks}."
    elif combined >= 0.35: feedback = f"Partial answer. More detail needed. Score: {score}/{max_marks}."
    elif combined >= 0.15: feedback = f"Minimal relevance to the question. Score: {score}/{max_marks}."
    else:                  feedback = f"Answer does not address the question. Score: {score}/{max_marks}."
    print(f"[Eval] SBERT={sbert_score:.2f} TFIDF={tfidf_score:.2f} KW={keyword_score:.2f} → {score}/{max_marks}")
...
def run_background_evaluation(student_id, exam_id, app_context):
    with app_context:
        try:
            ...
            cursor.execute("""
                SELECT a.id, a.answer, q.question_text, q.marks, q.question_type, q.correct_answer
            ...
            print(f"[BG Eval] Student {student_id}, Exam {exam_id} — {len(theory_answers)} answers")
            for i, ans in enumerate(theory_answers):
                ...
                if not student_answer or len(student_answer) < 10:
                    ...
                    score, feedback = evaluate_answer(ans['question_text'], student_answer, ans['marks'], model_answer=ans.get('correct_answer', ''))
...
# ============================================================
# EMAIL FUNCTIONS — EduSkills style (clean, minimal)
# ============================================================
...
def admin_dashboard():
    ...
    if admin_branch == "ALL":
        ...
        active_exams = cursor.fetchone()["total"]
        cursor.execute("SELECT COUNT(a.id) AS total FROM answers a JOIN exams e ON a.exam_id = e.id WHERE a.score IS NULL AND e.branch = %s", (admin_branch,))
        pending_ai_checks = cursor.fetchone()["total"]
    cursor.close()
    conn.close()
...
def add_student():
    ...
    if request.method == "POST":
        ...
        hashed_password = generate_password_hash("OEMS@12345")
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO students (name, admission_no, program, branch, semester, password) VALUES (%s,%s,%s,%s,%s,%s)", (name, admission_no, program, branch, semester, hashed_password))
        conn.commit()
        cursor.close()
        conn.close()
...
def create_exam():
    ...
    if request.method == "POST":
        ...
        conn   = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO exams (title, exam_type, total_marks, program, branch, semester, start_time, duration, status, browser_mode, ai_proctoring) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 'draft', %s, %s)", (title, exam_type, total_marks, program, branch, semester, start_time, duration, browser_mode, ai_proctoring))
        conn.commit()
        cursor.close()
        conn.close()
        return redirect("/admin")
...
def add_question(exam_id):
    ...
    if request.method == "POST":
        ...
        cursor.execute("INSERT INTO questions (exam_id, question_text, question_type, optionA, optionB, optionC, optionD, correct_answer, marks) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)", (exam_id, question_text, question_type, optionA, optionB, optionC, optionD, correct_answer, marks))
        conn.commit()
        cursor.close()
        conn.close()
...
def edit_question(question_id):
    ...
    if request.method == "POST":
        ...
        cursor.execute("UPDATE questions SET question_text=%s, optionA=%s, optionB=%s, optionC=%s, optionD=%s, correct_answer=%s, marks=%s WHERE id=%s", (question_text, optionA, optionB, optionC, optionD, correct_answer, marks, question_id))
        conn.commit()
...
def student_dashboard():
    ...
    return render_template("student_dashboard.html", student=student_info, exams=exams, attempted_exam_ids=attempted_exam_ids, now_time=now_time, timedelta=timedelta)
...
def start_exam(exam_id):
    ...
    if now < exam_start:
        cursor.close()
        conn.close()
        return f"<script>alert('Exam has not started yet. Starts at {exam_start.strftime('%I:%M %p')}'); window.location.href='/student';</script>"
    if now > exam_end:
        cursor.close()
        conn.close()
    ...
    return render_template("start_exam.html", questions=questions, exam_id=exam_id, exam=exam, exam_start=exam_start.strftime("%Y-%m-%dT%H:%M:%S"), exam_end=exam_end.strftime("%Y-%m-%dT%H:%M:%S"), duration=exam["duration"], ai_proctoring=ai_proctoring)
    ...
    for q in questions:
        ...
        if q_type == "mcq":
            ...
            if student_answer == q["correct_answer"]:
                score, feedback = q["marks"], "Correct answer"
            else:
                score, feedback = 0, "Incorrect or no answer"
            total_score += score
            cursor.execute("INSERT INTO answers (student_id, exam_id, question_id, answer, score, feedback) VALUES (%s,%s,%s,%s,%s,%s)", (student_id, exam_id, q["id"], student_answer, score, feedback))
        elif q_type == "msq":
            student_answers_list = sorted(answers_data.getlist(f"q{q['id']}"))
            student_answer = ", ".join(student_answers_list)
            db_correct_str = ", ".join(sorted([a.strip() for a in (q["correct_answer"] or "").split(",") if a.strip()]))
            if student_answer == db_correct_str and student_answer:
                score, feedback = q["marks"], "Correct answers"
            else:
                score, feedback = 0, "Incorrect or partially correct"
            total_score += score
            cursor.execute("INSERT INTO answers (student_id, exam_id, question_id, answer, score, feedback) VALUES (%s,%s,%s,%s,%s,%s)", (student_id, exam_id, q["id"], student_answer, score, feedback))
        else:
            has_theory     = True
            student_answer = answers_data.get(f"q{q['id']}", "").strip()
            cursor.execute("INSERT INTO answers (student_id, exam_id, question_id, answer, score, feedback) VALUES (%s,%s,%s,%s,%s,%s)", (student_id, exam_id, q["id"], student_answer, None, "Pending AI evaluation"))
    initial_status = "Pending" if has_theory else "Evaluated"
    cursor.execute("INSERT INTO results (student_id, exam_id, total_score, submission_status) VALUES (%s, %s, %s, %s)", (student_id, exam_id, round(total_score, 2), initial_status))
    conn.commit()
    cursor.close()
    conn.close()
    if has_theory:
        app_ctx = app.app_context()
        thread  = threading.Thread(target=run_background_evaluation, args=(student_id, exam_id, app_ctx), daemon=True)
        thread.start()
        print(f"[Submit] Background evaluation started for student {student_id}, exam {exam_id}")
...
def run_ai_check():
    conn   = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT answers.id, answers.answer, answers.student_id, answers.exam_id, questions.question_text, questions.marks, questions.correct_answer FROM answers JOIN questions ON answers.question_id = questions.id WHERE answers.score IS NULL AND questions.question_type = 'theory'")
    records = cursor.fetchall()
    print(f"[AI Check] Pending theory answers: {len(records)}")
    for r in records:
        student_answer = (r["answer"] or "").strip()
        if not student_answer or len(student_answer) < 10:
            cursor.execute("UPDATE answers SET score=%s, feedback=%s WHERE id=%s", (0, "No answer submitted or too short.", r["id"]))
            continue
        score, feedback = evaluate_answer(r["question_text"], student_answer, r["marks"], model_answer=r.get("correct_answer", ""))
...
def detect_cheating():
    ...
    try:
        ...
        face_center_x = x + w / 2
        face_center_y = y + h / 2
        x_offset = (face_center_x - img.shape[1]/2) / (img.shape[1]/2)
        y_offset = (face_center_y - img.shape[0]/2) / (img.shape[0]/2)
        ...
        if y_offset >  0.25: return jsonify({"cheating": True, "reason": "Looking Down (Suspicious)"})
...
class OTPManager:
    ...
    def store_otp(email, otp, mode='email', user_name=None):
        ...
        session['otp_data'] = {'otp': otp, 'email': email, 'mode': mode, 'user_name': user_name,
                               'expires_at': expiry_time.isoformat(), 'attempts': 0, 'created_at': datetime.now().isoformat()}
...
<table align="center" width="100%" style="max-width:500px;background:#fff;border-radius:8px;border:1px solid #e0e0e0;margin:0 auto;border-collapse:collapse;">
  ...
  <tr>
    ...
    def send_success_email(self, receiver_email, mode, user_name="Student"):
        """Success email after OTP verified — EduSkills style"""
        try:
            first_name = get_first_name(user_name)
            msg = MIMEMultipart('alternative')
            msg['From'] = formataddr(('OEMS Support Team', self.sender_email))
...
<table align="center" width="100%" style="max-width:500px;background:#fff;border-radius:8px;border:1px solid #e2e8f0;margin:0 auto;">
  ...
  <tr>
    ...
    def create_exam_alert_msg(self, receiver_email, student_name, exam_name, exam_date, duration):
        """Exam publish alert — EduSkills style"""
        first_name   = get_first_name(student_name)
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        msg = MIMEMultipart('alternative')
...
def student_result(exam_id):
    ...
    return render_template('student_result.html', student=student, exam=exam, answers=answers, total_score=total_score, max_marks=max_marks, percentage=percentage, grade=grade, total_questions=total_q, attempted=attempted, not_attempted=not_attempted, submission_status=result['submission_status'])
